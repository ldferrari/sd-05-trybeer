...
const createUser = ({ email, name, password, role }) =>
  connection.query('INSERT INTO users (email, name, password, role) VALUES (?,?,?,?)', [
    email,
    name,
    password,
    role,
...
const findUserById = (id) =>
  connection.query('SELECT id, name, email, role FROM users WHERE ID = ? ', [id]);

const updateUser = (id, name, email, password, role) =>
  connection.query('UPDATE users SET name = ?, email = ?, password = ?, role = ? WHERE id = ?', [
    name,
    email,
    password,
    role,
